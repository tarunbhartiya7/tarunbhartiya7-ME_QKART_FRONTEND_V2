  ...
  return (
    <Box display="flex" flexDirection="column">
      ...
      <Stack direction="row" my="1rem">
        <Button
          variant="contained"
          onClick={() => handleNewAddress(addAddress(token, newAddress))}
        >
          Add
        </Button>
        <Button
...
const Checkout = () => {
  ...
  const performCheckout = async (token, items, addresses) => {
    ...
    try {
      ...
      if (err.response) {
        ...
        enqueueSnackbar(
          "Could not place order. Ckeck that the backend is running, reachable and returns valid JSON",
          { variant: "error" }
        );
      }
    }
  ...
  }, [enqueueSnackbar, getAddresses, history, token]);
